...
import withShipperAuth from "../HOC/withShipperAuth";
...
const ShipperOrderList = () => {
  ...
  useEffect(() => {
    if (ordersData) {
      // Chỉ lấy các đơn hàng có status là "Ready for pickup" và "Delivering"
  ...
  const handleStatusChange = async (orderId: string, newStatus: string) => {
    ...
    try {
      ...
      // Cập nhật trạng thái trong state orders và ẩn đơn hàng sau khi chuyển trạng thái Completed
  ...
  const handleCompleteClick = (orderId: string) => {
    const userConfirmed = window.confirm("Đã hoàn thành việc giao hàng! ");
    if (userConfirmed) {
...
export default withShipperAuth(ShipperOrderList);
